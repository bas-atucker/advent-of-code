...
fn main() {
    ...
    for line in &lines {
        ...
        let calc_checksum = get_checksum(&parts[0]);

        if calc_checksum == parts[1] {
            let all_parts: Vec<&str> = line.split(|c| c == '[' || c == '-').collect();
            let sector_id = all_parts[all_parts.len() - 2].parse::<u32>().unwrap();
            let no_numeric = parts[0].replace(char::is_numeric, "");

            let msg = no_numeric
                .chars()
                .map(|c| roll(c, sector_id))
                .collect::<String>();

            println!("{}: {}", sector_id, msg);
        }
    }
}
...
fn get_checksum(text: &str) -> String {
    ...
    let ch = no_numeric.chars();
    let mut occurrences: BTreeMap<char, i32> = BTreeMap::new();
    let mut strings: BTreeMap<i32, String> = BTreeMap::new();
    let mut ordered_str: String = String::from("");
    ...
    for (k, v) in occurrences.iter()  {
        *strings.entry(-1*v).or_insert(String::from("")) += &k.to_string();
    }

    for (_, v) in strings.iter() {
        ordered_str += &v;
    }

    String::from(&ordered_str[..5])
}

const INPUT: &'static str = r#"aczupnetwp-mfyyj-opalcexpye-977[peyac]
